...
Let $P_s^*$ be the reference substation net and $c_i^*$ the signed reference
contribution of equipment $i$. The private factor file stores

$$
f_i =
\begin{cases}
c_i^*/|P_s^*|, & |P_s^*|>\varepsilon,\\
c_i^*, & |P_s^*|\leq\varepsilon,
\end{cases}
...
c_i^* =
\begin{cases}
f_i|P_s^*|, & |P_s^*|>\varepsilon,\\
f_i, & |P_s^*|\leq\varepsilon.
\end{cases}
$$

Therefore $\hat P_s=P_s^*$ reproduces the reference dispatch.

For nonzero-throughput substations, equipment roles follow $c_i^*$ rather than
equipment type:

| Reference contribution | Role |
|---:|---|
| $c_i^*>0$ | Positive injector |
| $c_i^*<0$ | Negative injector |
| $c_i^*=0$ | Zero-reference unit |
...
assets. Their excess never contributes to the ordinary asset count $N^{lim}$.
Instead, minor and severe thermal violations contribute to $N^{ov,1}$ and
$N^{ov,2}$.
...
$$
u_i^{load}=\max\left(0,\max_{t\in T_i}c_{i,t}^*\right)+\eta,
$$
...
Every public load with $c_i^*>0$ in a scored snapshot must have exactly one row
...
Water-filling distributes $\hat P_s-P_s^*$ using fixed weights
$w_i=|c_i^*|$. A unit reaching its relevant limit is clamped; the remainder is
...
| Finite boundary route | Keep ordinary generators at `maxP`; share residual over the boundary assets by directional thermal limit, producing only $N^{ov,1}$/$N^{ov,2}$ |
| No boundary route, connected ordinary generators | Share residual over every connected ordinary generator by `maxP`; every generator, including zero-reference generators, adds one to $N^{lim}$ |
| Neither boundary route nor generator | Share the residual over connected loads in proportion to their synthetic upper limits; if all limits are zero, share it equally. Loads may exceed their synthetic limits without contributing to $N^{lim}$ |
...
|---|---|---|
| 1 | Within nonzero-reference range | Water-fill over connected nonzero-reference equipment using $|c_i^*|$ |
...
| 3 | Above all connected upper capacity | Overfill every connected ordinary generator by `maxP`; all generators cross together and each adds one to $N^{lim}$. Positive loads remain at their synthetic limits unless no generator exists |
| 4 | Below all connected lower capacity | Use negative-capable ordinary units, then positive-capable ordinary units through zero if necessary; every overfilled ordinary generator adds one to $N^{lim}$ |
...
| 3 | Below finite negative capacity | Use negative-capable boundary assets first; otherwise use negative-capable ordinary units, then positive-capable ordinary units through zero. Every overfilled ordinary generator adds one to $N^{lim}$ |
...
| Regime | Condition | Allocation and scoring |
|---|---|---|
| 1 | Within nonzero-reference range | Water-fill over connected nonzero-reference ordinary and boundary units using $|c_i^*|$. Finite units clamp individually; unbounded units do not clamp |
...
| 3 | Above all finite upper limits | Keep generators and positive loads at their limits; share residual over finite positive-direction boundary assets by thermal limit. They enter $N^{ov,1}$ and then $N^{ov,2}$ together. If an unbounded boundary asset exists, it absorbs the residual without a violation |
| 4 | Below all finite lower limits | Keep ordinary units at lower limits; share residual over finite negative-direction boundary assets by thermal limit. They enter $N^{ov,1}$ and then $N^{ov,2}$ together. If an unbounded boundary asset exists, it absorbs the residual without a violation |

If no boundary asset is usable in the overflow direction, the corresponding
Case 1 overflow rule applies. Boundary assets never contribute to $N^{lim}$.
